...
bool resetLocation(const char* udid) {
    ...
    if (IDEVICE_E_SUCCESS != idevice_new(&phone, udid)) {
        printf("ERROR: No iOS device with specified udid found.\n");
        goto leave_and_cleanup;
    }

    if (LOCKDOWN_E_SUCCESS != lockdownd_client_new_with_handshake(phone, &client, "devicelocation")) {
        printf("ERROR: Could not connect to lockdownd client.\n");
        goto leave_and_cleanup;
    }

    if ((lockdownd_start_service(client, "com.apple.dt.simulatelocation",  &service) != LOCKDOWN_E_SUCCESS) || !service) {
        printf("ERROR: Could not start com.apple.dt.simulatelocation!\n");
        goto leave_and_cleanup;
    }

    if (service_client_new(phone, service, &service_client)) {
        printf("ERROR: Could not create service client.\n");
        goto leave_and_cleanup;
    }
    ...
    {
        printf("ERROR: Could not send stop data to service client.\n");
        goto leave_and_cleanup;
    }
    res = true;
...
bool sendLocation(const char *lat, const char *lng, const char* udid) {
    ...
    if (LOCKDOWN_E_SUCCESS != lockdownd_client_new_with_handshake(phone, &client, "devicelocation")) {
        printf("ERROR: Could not connect to lockdownd.\n");
        goto leave_and_cleanup;
    }
    ...
    if (service_client_new(phone, service, &service_client)) {
        ...
        if (service_send(service_client, (void*)&startMsg, sizeof(startMsg), &send_bytes)
            ...
            printf("ERROR: Could not send start data to service client.\n");
            goto leave_and_cleanup;
        }
        ...
        {
            printf("ERROR: Could not send lat data to service client.\n");
            goto leave_and_cleanup;
        }
        ...
        {
            printf("ERROR: Could not send lng data to service client.\n");
            goto leave_and_cleanup;
        }
        res = true;
    }
