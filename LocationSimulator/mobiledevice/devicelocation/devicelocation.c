...
bool resetLocation(const char* udid) {
    ...
    {
        LOG_ERR("Could not send stop data to service client.");
        goto leave_and_cleanup;
    }
    res = true;
...
bool sendLocation(const char *lat, const char *lng, const char* udid) {
    ...
    if (LOCKDOWN_E_SUCCESS != lockdownd_client_new_with_handshake(device, &client, "devicelocation")) {
        LOG_ERR("Could not connect to lockdownd.");
        goto leave_and_cleanup;
    }
    ...
    if (service_client_new(device, service, &service_client)) {
        ...
        if (service_send(service_client, (void*)&startMsg, sizeof(startMsg), &send_bytes)
            ...
            LOG_ERR("Could not send start data to service client.");
            goto leave_and_cleanup;
        }
        ...
        {
            LOG_ERR("Could not send lat data to service client.");
            goto leave_and_cleanup;
        }
        ...
        {
            LOG_ERR("Could not send lng data to service client.");
            goto leave_and_cleanup;
        }
        res = true;
    }
