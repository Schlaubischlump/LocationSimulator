    ...
    static func startGeneratingDeviceNotifications() -> Bool {
        ...
        let callback: idevice_event_cb_t = { (event, userData: UnsafeMutableRawPointer?) in
            ...
            case CONNECTION_NETWORK:
                ...
                guard Device.detectNetworkDevices else { return }
                conType = .network
            ...
            case IDEVICE_DEVICE_ADD, IDEVICE_DEVICE_PAIRED:
                ...
                if (device != nil) && !(device!.connectionType.contains(conType)) {
                    ...
                    device = Device(UDID: udid, name: String(cString: res), connectionType: conType)
...
extension Device: CustomStringConvertible {
    var description: String {
        return "Device(udid: \(self.udid), name: \(self.name), connectionType: \(self.connectionType))"
    }
}
